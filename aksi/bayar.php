...
if($_POST){
    if($_POST['aksi']=='tambah'){  //untuk manajemen
        ...
        $sql="UPDATE biaya SET id_biaya='$id_biaya',id_periode='$id_periode',tingkat='$tingkat',id_jurusan='$id_jurusan', deskripsi_biaya='$deskripsi_biaya',jumlah_biaya='$jumlah_biaya',tanggal_jatuh_tempo='$tanggal_jatuh_tempo' WHERE id_biaya=$id_biaya";
        //  echo $sql; // Cek Perintah
        mysqli_query($koneksi,$sql);
        notifikasi($koneksi);
    ...
    if($_GET['aksi']=='hapus'){
      $id_bayar=$_GET['id_bayar'];
       // $sql="DELETE FROM bayar WHERE id_bayar=$id_bayar"; //Hard Delete
        ...
        $query3=mysqli_fetch_array($koneksi,$query3);
        ...
        $id_bayar=$_GET['id_bayar'];
        $sql="DELETE FROM biaya WHERE id_bayar=$id_bayar";
         mysqli_query($koneksi,$sql);
         notifikasi($koneksi);
         header('location:../index.php?p=biaya');
    }
